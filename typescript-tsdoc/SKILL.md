...
- **Private remarks:** внутренние заметки для разработчиков — через `@privateRemarks`, не
  в `@remarks`.
- **Никаких голых `<...>` в тексте.** TSDoc-парсер (и markdown-рендеры на его базе)
  интерпретирует `<button>`, `<a>`, `<T>`, `<MyComponent>` как HTML/JSX-теги: текст либо
  вырезается из вывода, либо ломает парсинг блока (особенно опасно для непарных вхождений вроде
  «при `tag="a"`» рядом с «`<a>`»). Любые угловые скобки в описаниях — **внутри inline-кода**:
  ``` `<button>` ```, ``` `<a href>` ```, ``` `Array<T>` ```. Если inline-код неуместен —
  заменить на текстовое описание («тег-якорь», «тег кнопки») или экранировать entity-ами
  (`&lt;button&gt;`). Правило распространяется на summary, `@remarks`, описания `@param`,
  `@returns`, `@typeParam` и любые другие текстовые блоки.
...
- [ ] В описаниях нет голых `<...>` — все угловые скобки обёрнуты в inline-код или
  заменены текстом / entity-ами.
